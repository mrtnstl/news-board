...
export class PuppeteerScraper<T> implements IScraper<T> {
    private browser: Awaited<ReturnType<VanillaPuppeteer["launch"]>> | undefined;
    ...
    async init(){
        try{
            ...
            throw new ErrorWithCause("Failed to init browser", new Error(String(err)));
        }
    }
    async scrape(): Promise<T[]>{
        try{
            if(!this.page){
                throw new Error("Data retrieval can not proceed due to failed initialization");
            }
            if(this.options?.rawFields == undefined || Object.keys(this.options.rawFields).length < 1){
            ...
            while(this.options.currentPage <= this.options.maxPages){
                ...
                if(this.options.url.includes(PAGEN_NUMBER_TEMPL)){    
                    url = this.options.url.replace(PAGEN_NUMBER_TEMPL, String(this.options.currentPage));
                } else {
                ...
                if(this.options.selectorToWaitFor){
                    await this.page.waitForSelector(this.options.selectorToWaitFor, {
                        timeout: 30000,
                    });
                }
                
                const rawProducts = await this.page.evaluate((root, fieldDefinitions) => {
                    const items = document.querySelectorAll(root);

                    return Array.from(items).map((container, index) => {
                        const result: Record<string, string> = { index: `${index}` };

                        for (const [fieldName, definition] of Object.entries(fieldDefinitions)) {
                            const element = container.querySelector(definition.selector);

                            if (definition.attribute) {
                                result[fieldName] = element?.getAttribute(definition.attribute)?.trim() 
                                || definition.fallback || "";
                            } else {
            ...
            throw new ErrorWithCause("Failed to scrape source", new Error(String(err)));
        }
    }
    async cleanup(){
        if (this.browser) {
