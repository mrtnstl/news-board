...
const NYTimesSrcaper = new PuppeteerScraper<TNYTimesResponse>({
    ...
    rawFields: {
        ...
        publishedAt: {
            selector: 'div.e15t083i3 span[data-testid="todays-date"]',
            fallback: "no_data",
        },
        articleLink: {
            selector: ".css-1l4spti a.css-8hzhxf",
            attribute: "href",
        },
...
const BleepingComputerScraper = new PuppeteerScraper<TBleepingComputerResponse>(
    {
        ...
        rawFields: {
            ...
            imageURL: {
                selector: ".bc_latest_news_img a img",
                attribute: "src",
            },
            publishedAt: {
                selector: ".bc_latest_news_text ul .bc_news_date",
                fallback: "no_data",
            },
            articleLink: {
...
(async () => {
    try {
        ...
        if (err instanceof Error) {
            console.log(
                new ErrorWithCause("Unexpected error", err).unwrapCauses(),
            );
        }
    } finally {
